...
async def calibrator_planning():
    ...
    supervisor = SupervisorAgent(
        config=supervisor_agent_config, message_bus=message_bus, llm=llm
    )
    calibrator = CalibratorAgent(
        config=calibrator_agent_config, message_bus=message_bus, llm=llm
    )

    req = """支撑省内集团客户部对外大数据服务合作项目，需要用户行业网关短信收发时间明细数据的支撑。需包含集团客户名称，行业子类型名称，服务代码，第三方电话，短信发送状态，处理结束日期，处理结束时间，统计日期等信息。"""

    print("Processing task with SupervisorAgent & CalibratorAgent...")
    try:
        ...
        print(
            "SupervisorAgent processed results, passing to calibrator:",
            msg_for_calbrator,
        )

        msg_for_calibrator_str = json.dumps(
            msg_for_calbrator, ensure_ascii=False, indent=2
        )
